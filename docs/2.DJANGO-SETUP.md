...
Within the realm of Python and web frameworks, the two that commonly arise in conversation are [Django]() and [Flask]().
In this learning path, Django will be used as there is some preexisting material on Flask in a series
called [Python like a Pro](). Although these open source web frameworks have their similarities and differences, both
are designed to simplify the programming experience for developers building web applications. For more information on
Flask or comparisons of web frameworks, there is a multitude of online sources available describing the two frameworks
in depth

### Django vs. Django REST Framework

Now that you understand, at least the basics, of Django - you might be wondering "What is the difference between Django
and Django REST Framework?". Django, at the very basic level, is the web framework that simplifies the creation of web
apps. Django REST Framework (Sometimes simplified as DRF) is a dependency you can use that adds functionality to Django
to simplify using Django for a REST API.
...
Another benefit of using Django is that it often is called "Batteries Included" framework; for example, it comes
pre-configured with a ready to use SQLite database. As this series will be creating a [CRUD Application](), you will
...
The one with the .`venv` folder), run:
...
for now, it will be brought up again later in the series. This will not affect your current running application.
...
be treated as a Python package that will contain all of your Django conventions.
...
folder for your application with all of the application scaffolding. The application file structure should look like:
...
to make it easy to understand. In the next section, you will begin to test and build your REST API using this
scaffolding.
