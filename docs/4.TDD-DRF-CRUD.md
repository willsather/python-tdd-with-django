...
Refactor" process; however, it is recommended as you follow along to work incrementally and experiment with what makes
...
Not that you have a failing test, to implement the `POST` endpoint, in your `views.py` `PenguinController` you can
write:
...
and `Create` in CRUD), but now you need to implement the `Update` and `Delete` aspects of the endpoint.

However, your application is at a point where you would now need to create a new view for `Penguin Detail` that would
look something like this when calling it from a URL: `/api/penguin/1` where `pk` is `1` which is how you retrieve,
update, and delete a single instance of a `Penguin` object.
...
ability to quickly create RESTful views using _out-of-the-box_ functionality.  
CRUD endpoints are quite common and thus, Django REST Framework have made a variety of Views, like the one you are using
now: `RetrieveUpdateDestroyAPIView`.
...
simplicity and abstraction from the REST implementation. This allows you to worry less about the "How REST" and more
about what you are actually building.

Up until this point, we have been doing TDD through writing a failing test, then changing the implementation to get the
test to pass. Now, a critical piece of TDD is refactoring. As the saying goes: "Red, Green, Refactor". Now that you have
some test written that test the behavior of your API, you can change the implementation or refactor; and by running your
...
From this point on, this learning path will be using the condensed `PenguinController` to save space and for less
complexity.

## Wrap Up

At the end of this learning path, you have now built a basic CRUD application using Django / Django REST Framework all
why using TDD. Using TDD allows you to refactor and make new changes while still validating it hasn't broken other parts
of you code.

This CRUD application is able to perform `HTTP requests` and provides `HTTP Responses` all in regard to the `Penguin`
object. For example, if you make a few `POST` requests with new `Penguin` objects, that data will persist in the
pre-configured database from Django.
